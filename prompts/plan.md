...
## 3. Review four ways
Dispatch four reviewer subagents if your runtime supports them; otherwise run the four reviews yourself, one after another, each a fresh pass over the spec and the tickets.
Give each review one brief below, and fold the findings back into the
tickets:
