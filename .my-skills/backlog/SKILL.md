...
**Track — at most one, applied by the daily triage sweep:**

`track:<name>` for every track named under `models.tracks` in the box's
`targets.yaml`. Decides which model writes the spec; the spec session picks
the track for the later stages. `setup` creates one label per configured
track name (pass them as `--tracks trivial,standard,frontend,security`).
...
2. Follow `references/graphql.md` → **Setup**: create the Project, link the repo,
   ...
   (idempotent — `--force` reconciles color/description on re-runs), plus one `track:<name>` label per name given with `--tracks`. On a
