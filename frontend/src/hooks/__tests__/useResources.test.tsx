...
it('useTaskDetail(widget, 42) resolves the parked task', async () => {
  const { result } = renderHook(() => useTaskDetail('widget', 42), { wrapper })
  await waitFor(() => expect(result.current.isSuccess).toBe(true))
...
it('useTaskHistory fetches only when enabled', async () => {
  ...
  const { result, rerender } = renderHook(
    ({ on }: { on: boolean }) => useTaskHistory('widget', 42, on),
